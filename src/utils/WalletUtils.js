...
const aegs = {
	messagePrefix: '\x19Aegisum Signed Message:\n',
	bip32: {
	  public: 0x0488b21e,
  	  private: 0x0488ade4
	},
...
function cltvOutput(address, lockTime) {
	return bitcoin.script.compile([
	  ...
	  bitcoin.address.fromBase58Check(address, aegs).hash,
...
function getP2WPKHScript(publicKey) {
  return bitcoin.payments.p2wpkh({ pubkey: publicKey, network: aegs });
}
...
export function isAddress(address) {
	try {
		bitcoin.address.fromBase58Check(address, aegs)
		return true
	} catch (e) {
		try {
			bitcoin.address.fromBech32(address, aegs)
			return true
		} catch (e) {
			return false
		}
...
export function importAddressByWIF(wif) {
	try {
		const keyPair = bitcoin.ECPair.fromWIF(wif, aegs);
		return getAddress(keyPair, aegs);
	} catch (e) {
		return null;
	}
}
...
export function generateAddresses(seedPhrase, derivePath = "m/44'/0'/0'/0", startIndex = 0, endIndex = 0) {
	...
	const root = bip32.fromSeed(seed, aegs);
    let addressList = {};
    let promises = [];

    for (var i = startIndex; i <= endIndex; i++) {
		const child = root.derivePath(derivePath + "/" + i.toString());
	    addressList[getAddress(child, aegs)] = {index: i, privateKey: child.toWIF()};
    }
...
export async function sendTransation(socketConnect, walletAddresses, mainAddress, recieveAddress, amount, fee, timelock = 0) {
	...
	var keyPairs = [];
	var scripts = [];
    const txb = new bitcoin.TransactionBuilder(aegs)
		...
		if (balance != null && (balance.balance > 0)) {
			try {
				...
				for (var k = 0; k < utxo.length; k++) {
					...
					keyPairs.push(bitcoin.ECPair.fromWIF(walletAddresses[address].privateKey, aegs));
